...
class UploadServer:
    ...
    def handle_client(self, client_socket):
        # Handles an individual client's download or torrent piece download request.
        try:
            ...
            if data:
                ...
                # Process a torrent piece request
                elif request.get("type") == "PIECE_REQUEST":
                    filename = request.get("filename")
                    ...
                    if filename is None or piece_index is None:
                        response = {
                            "type": "PIECE_RESPONSE",
                            "status": "ERROR",
                            "error": "Missing filename or piece_index."
                        }
                    ...
                    # Define default piece size (512 KB)
                    piece_size = 512 * 1024
                    file_size = os.path.getsize(file_path)
                    offset = int(piece_index) * piece_size
                    if offset >= file_size:
                        response = {
                            "type": "PIECE_RESPONSE",
                            "status": "ERROR",
